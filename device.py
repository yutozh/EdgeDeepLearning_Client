...
def getDeviceInfo(identity=''):
    cpu = GetCpuConstants().get('cpu_name', '')
    memory = "{:.2f}".format(GetMemInfo().get('memTotal', '') / 1024)
    # os = sys.platform
    os = platform.system()
    mac = uuid.UUID(int = uuid.getnode()).hex[-12:]
    name = socket.gethostname()
    return {
        "uni_id": md5(mac + name + identity),
        "name": name,
        "cpu": cpu,
        "memory": memory,
        "os": os,
        "device_type": "树莓派",
        "data_meta": "CIFAR-10 图像" # TODO: 识别数据集
    }

def getRealtimeDeviceInfo():
    cpu_ing = GetCpuInfo().get('used', 0)
    memory_ing = "{:.2f}".format(GetMemInfo().get('memRealUsed', 0) / 1024)
    memory = "{:.2f}".format(GetMemInfo().get('memTotal', 1) / 1024)
    return {
        "cpu_ing": cpu_ing,
